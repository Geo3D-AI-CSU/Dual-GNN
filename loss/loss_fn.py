...
def scalar_loss(predicted_scalar, rock_unit_labels, min_values, max_values):
    ...
    - rock_unit_labels (Tensor): 岩性标签，形状为 (num_nodes, )，用于查找对应的范围。 从1开始，后续会自动减去1，因为交叉熵那个函数要从0开始索引。
    ...
    如rock_unit_labels=[1,2,3,4],min_values=[-9999,10,40,80],max_values=[10,40,80,-9999]
    代表岩性1的最大值为10；岩性2的最小值为10，最大值为40；岩性3的最小值为40，最大值为80；岩性4的最小值为80。
    返回：
