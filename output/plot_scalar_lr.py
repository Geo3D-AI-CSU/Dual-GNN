...
def plot_lr(file_path, save_path=None):
    ...
    with open(file_path, 'r', encoding='utf-8') as f:
        for line in f:
            # 使用正则表达式提取 Epoch 和 LR
            match = re.search(
                r"Epoch\s*(\d+/\d+).*?LR\s*=\s*([\d.]+)",
                line.strip()
            )
            if match:
                epoch = match.group(1).split('/')[0]  # 提取当前周期
                lr = float(match.group(2))
                data.append([int(epoch), lr])
    ...
    plt.subplots_adjust(left=0.2, right=0.9, top=0.9, bottom=0.15)  # 调整图框范围
    plt.plot(df['Epoch'], df['LR'], label='Learning Rate', color='#006300', marker='o', markersize=6)
    plt.xlabel('训练周期', fontsize=40)
    plt.ylabel('学习率', fontsize=40)
...
if __name__ == '__main__':
    ...
    save_path=f'../tetra_output_files/{dataset_name}/scalar_lr.png'
