...
from model.model import GATLevelPredictor, GATRockPredictor
...
def train_and_validate(graph_data,min_values, max_values,num_epochs=300,lr=0.01,hidden_channels=128,num_classes=7,result_dir=None,dropout=dropout,lr_decay=0.8):
    ...
    # 初始化 GradNorm
    grad_norm = GradNorm_2loss(alpha=1.0, gamma=1.0, device=device)
    ...
    with open(log_scalar_file, 'w') as f:
        for epoch in range(1, num_epochs + 1):
            ...
            # scalar_loss_val = scalar_loss(predicted_level[mask_rock_unit], graph_data.rock_unit[mask_rock_unit]-1, min_values_normalized,
...
def main(node_file, ele_file,vtk_file,epoch=300,
         ...
         factor=1.0,dropout=0,is_gradient = True,lr_decay=0.8):
...
if __name__ == "__main__":
    # 读取实验参数配置文件
    with open('训练设置参数/0401Fault_cut_GAT.json', 'r') as f:
        experiments = json.load(f)
    # 遍历每组实验参数并运行
    for experiment_params in experiments:
        experiment_params["result_dir"] = (
            f"../tetra_output_files/Fault_cut/0407regularGATcut_fault{experiment_params['epoch']}epoch_"
