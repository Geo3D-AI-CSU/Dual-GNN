...
from model.model import LevelPredictor, GATRockPredictor,RockUnitPredictor
# LevelPredictor
...
def train_and_validate(graph_data,min_values, max_values,num_epochs=300,lr=0.01,hidden_channels=128,num_classes=7,result_dir=None,lr_decay=0.8):
    # 初始化模型并移动到设备
    model =  LevelPredictor(graph_data.x.size(1),hidden_channels=hidden_channels).to(device)
    ...
    # # 提取属性为Conformity的节点进行level损失计算
    # conformity_nodes = extract_conformity_nodes(graph_data)
    # mask_level = torch.zeros_like(graph_data.level, dtype=torch.bool).to(device)
    # mask_level[conformity_nodes] = True
    # mask_level = mask_level.to('cpu')
    mask_level =graph_data.mask_level

    # 提取属性为Rock_unit的节点进行level损失计算
    # rock_nodes = extract_rock_nodes(graph_data)
    # mask_rock_unit = torch.zeros_like(graph_data.rock_unit, dtype=torch.bool).to(device)
    # mask_rock_unit[rock_nodes] = True
    # mask_rock_unit = mask_rock_unit.to('cpu')
    mask_rock_unit = graph_data.mask_rock_unit

    # 提取属性为Horizon_occurence的节点进行gradient损失计算
    # horizon_nodes = extract_horizon_nodes(graph_data)
    # mask_gradient = torch.zeros_like(graph_data.level, dtype=torch.bool).to(device)
    # mask_gradient[horizon_nodes] = True
    # mask_gradient = mask_gradient.to(device)
    ...
    # 初始化 GradNorm
    # grad_norm = GradNorm_3loss(alpha=1.0, gamma=1.0, delta=0.5, device=device)
    grad_norm = GradNorm_2loss(alpha=1, gamma=1,device=device)
...
if __name__ == "__main__":
    # 读取实验参数配置文件
    with open('训练设置参数/0401Fault_cut_GraphSAGE.json', 'r') as f:
        experiments = json.load(f)
    # 遍历每组实验参数并运行
    for experiment_params in experiments:
        experiment_params["result_dir"] = (f"../tetra_output_files/Fault_cut/0407regular_GAT+GraphSAGE_nocut_fault_epoch_{experiment_params['epoch']}"
