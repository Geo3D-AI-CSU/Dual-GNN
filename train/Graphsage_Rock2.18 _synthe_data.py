...
def train_and_validate(graph_data,min_values, max_values,num_epochs=300,lr=0.01,hidden_channels=128,num_classes=7,result_dir=None,lr_decay=0.8):
    ...
    # Extract nodes with the attribute "Conformity" for level loss calculation.
    mask_level =graph_data.mask_level

    # Extract nodes with the attribute "Rock_unit" for level loss calculation.
    mask_rock_unit = graph_data.mask_rock_unit

    #  Extract nodes with the attribute "Horizon_occurrence" for gradient loss calculation.
    ...
    # Determining the number of fault characteristics
    ...
    with open(log_scalar_file, 'w') as f:
        for epoch in range(1, num_epochs + 1):
            ...
            # Compute Level loss (for masked nodes only)
    ...
    with open(log_scalar_file, 'a') as f: 
        ...
        with torch.no_grad():
            ...
            # Combine the predicted level with the x feature of the original image
    ...
    with torch.no_grad():
        # Using trained Level predictions and raw features as input for lithological prediction
        rock_unit_input = torch.cat([predicted_level_expanded , graph_data.x.to(device)], dim=-1)
        predicted_rock_units = rock_unit_model(rock_unit_input, graph_data.edge_index.to(device))
...
if __name__ == "__main__":
    # Load the experimental parameter configuration file
    with open('Training parameter settings/0401Fault_cut_GraphSAGE.json', 'r') as f:
