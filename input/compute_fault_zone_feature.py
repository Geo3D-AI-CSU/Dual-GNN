...
def compute_fault_features(graph_data, vtk_directory,factor=1.0):
    ...
    for idx, (level, mesh) in enumerate(sorted(fault_meshes.items())):
        ...
        try:
            ...
            for coord in coords:  # coords should be the node coordinates within the graph data
            ...
            # To calculate the distance from a computational node to a fault plane, the correct parameter order is: the fault plane as the first parameter, and the point set as the second parameter.
            ...
            fault_sides = (distances > 0).astype(int) 
